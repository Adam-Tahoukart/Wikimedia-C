        ...
        public ActionResult DeleteUser(int id)
        {
            if (id != 1)
            {
                ...
                if (user != null)
                {
                    ...
                    // Effacer tous les likes de l'usager
                    foreach (var like in DB.Likes.ToList().Where(l => l.UserId == id).ToList())
                        DB.Likes.Delete(like.Id);

                    // Effacer tous les médias de l'usager et leurs likes
                    foreach (var media in DB.Medias.ToList().Where(m => m.OwnerId == id).ToList())
                    {
                        foreach (var like in DB.Likes.ToList().Where(l => l.MediaId == media.Id).ToList())
                            DB.Likes.Delete(like.Id);
