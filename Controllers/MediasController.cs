...
public class MediasController : Controller
{
    ...
    public ActionResult GetMediaDetails(bool forceRefresh = false)
    {
        try
        {
            ...
            if (DB.Users.HasChanged || DB.Medias.HasChanged || DB.Events.HasChanged || forceRefresh || mediaId > 0)
            {
    ...
    public ActionResult GetDetailsHeader(bool forceRefresh = false)
    {
        try
        {
            InitSessionVariables();
            int mediaId = (int)Session["CurrentMediaId"];
            if (DB.Users.HasChanged || DB.Medias.HasChanged || DB.Events.HasChanged || forceRefresh)
            {
                ViewBag.PageTitle = "Détails";
                ViewBag.HeaderAction = "details";
                ViewBag.HeaderController = "medias";
    ...
    public ActionResult ToggleMediaLike(int id)
    {
        ...
        Like existingLike = DB.Likes.GetLike(id, Models.User.ConnectedUser.Id);

        if (existingLike != null)
            DB.Likes.Delete(existingLike.Id);
        else
            DB.Likes.Add(new Like { MediaId = id, UserId = Models.User.ConnectedUser.Id });
    ...
    public ActionResult Delete()
    {
        ...
        if (id != 0)
        {
            ...
            if (Media != null)
            {
                if (Media.OwnerId == Models.User.ConnectedUser.Id || Models.User.ConnectedUser.IsAdmin)
                {
                    // Effacer tous les likes du média avant d'effacer le média
                    foreach (var like in DB.Likes.ToList().Where(l => l.MediaId == id).ToList())
